...
public class CustomDialog extends AlertDialog {
    ...
    protected CustomDialog(@NonNull Context context, boolean cancelable, @Nullable
            OnCancelListener cancelListener) {
        super(context, cancelable, cancelListener);
    }

    public CustomDialog(Context context, String title, String message, boolean hasInput, String
            reject, String accept, OnClickListener onReject, OnClickListener onAccept, @Nullable
            OnCancelListener onCancelListener, boolean isCancelable, boolean isTypeReject) {
        super(context);
    ...
    public CustomDialog(Context context, @StringRes int title, @StringRes int message, boolean
            hasInput, @StringRes int reject, @StringRes int accept, OnClickListener onReject,
                        OnClickListener onAccept, @Nullable OnCancelListener onCancelListener,
                        boolean isCancelable, boolean isTypeReject) {
        super(context);
        if (onCancelListener != null) {
            setOnCancelListener(onCancelListener);
        }
        setCancelable(isCancelable);
        initView(context.getString(title), context.getString(message), hasInput, context
                .getString(reject), context.getString(accept), onReject, onAccept, isTypeReject);
    }

    private void initView(String title, String message, boolean hasInput, String reject, String
            accept, final OnClickListener onReject, final OnClickListener onAccept, boolean
            isTypeReject) {
        mBinding = LayoutCustomDialogBinding.inflate(getLayoutInflater());
        if (isTypeReject) {
            Context context = getContext();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                mBinding.toolbar.setBackgroundColor(context.getResources().getColor(R.color
                        .dialogPink, null));
                mBinding.title.setTextColor(context.getResources().getColor(R.color
                        .dialogTextRed, null));
                mBinding.message.setTextColor(context.getResources().getColor(R.color
                        .grey_bright, null));
            ...
            int dimen = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, context
                    .getResources().getDimension(R.dimen.dp_24), context.getResources()
                    .getDisplayMetrics());

            LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) mBinding.imageAlert
                    .getLayoutParams();
            lp.width = dimen;
            lp.height = dimen;
            ...
            dimen = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, context
                    .getResources().getDimension(R.dimen.dp_20), context.getResources()
                    .getDisplayMetrics());
            LinearLayout.LayoutParams lp1 = (LinearLayout.LayoutParams) mBinding.close
                    .getLayoutParams();
            lp1.width = dimen;
            lp1.height = dimen;
