...
import com.viettel.mbccs.screen.manager.area.holder.AreaChanelHolder;
...
public class AreaAdapter extends GenericRecycleAdapter<Area, AreaChanelHolder> {

    public AreaAdapter(List<Area> mList, Context context) {
        super(mList, context);
    }

    @Override
    public int getLayout(int viewType) {
        return R.layout.item_area_channel;
    }

    @Override
    protected RecyclerView.ViewHolder getHolderViewHolder(View v, int viewType) {
        return new AreaChanelHolder(v) {
            @Override
    ...
    @Override
    protected void onSet(Area item, AreaChanelHolder holder, int position) {
        if (item != null) {
            holder.render(item);
        }
    }
